#This is for aggregating all the separate matrices of the annular analysis in a csv that can then be further analyzed.
#Run after pre_damage_oct_annular_analysis
#OCT only?
...
def find_parameters(top_path):
    ...
    seek = '{}{}**{}background_subtracted_matrix.npy'.format(top_path, os.sep,os.sep)
    params_paths = glob.iglob(seek, recursive=True)
    
    with open('{}{}predmg_all_data.csv'.format(top_path, os.sep), 'w') as f:
        ...
        for path in params_paths:
            ...
            #load the background subtracted annular measurements
            anal_mat = np.load(path)
            #IS/OS
            for depth in range(10,36):
                for rad in range(0,60):
                    csvf.writerow(path_info+[depth,rad,anal_mat[depth, rad]])
            #ONL
