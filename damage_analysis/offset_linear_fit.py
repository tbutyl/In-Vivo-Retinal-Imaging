...
def find_data(top_path):
	...
	#background subtration test - fit 30-60 rings to y=0*x+a
	...
	for path in mat_paths:
		...
		for row in analysis_mat:
			...
			depth+=1
		combo = np.concatenate((analysis_mat, profile_fits), axis=1)
		np.save('{}{}linear_parameters.npy'.format(save_path, os.sep), combo)
		del profile_fits
