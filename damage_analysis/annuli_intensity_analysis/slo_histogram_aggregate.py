...
def agg(top_path):
    seek = '{}{}**{}histogram_annuli.npy'.format(top_path, os.sep,os.sep)
    paths = glob.iglob(seek, recursive=True)

    with open('{}{}slo_annuli_histogram_data.csv'.format(top_path, os.sep), 'w') as f:
        csvf = csv.writer(f,delimiter=',')
        csvf.writerow(['Set', 'Date', 'Mouse Number', 'Eye','Genotype', 'Intensity', 'Count'])      
        for path in paths:
            print('\n{}'.format(path))
            arr = np.load(path)
            path_info = procPath(path)
            for inten in arr:
                csvf.writerow(path_info+[inten[0], inten[1]])
