...
def main():
	...
	if args.filePath and args.inputFile:
		filePath=str(args.filePath)
		inputFile=str(args.inputFile)
	else:
		fullPath=askopenfilename()
		fullPathList = str.rsplit(fullPath, os.path.sep,1)
		fileSep = os.path.sep
		#I have this contingency in here because the exFat drive appears to use /, while windows is still using \\
		if len(fullPathList)<2:
			fullPathList = str.rsplit(fullPath,'/',1)
			fileSep = '/'
			if len(fullPathList)<2:
				fullPathList = str.rsplit(fullPath,'\\',1)
				fileSep = '\\'
		filePath = fullPathList[0] + fileSep
		inputFile = fullPathList[1]
	if args.fileName:
